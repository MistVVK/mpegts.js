...
| `forceMSEStreamLivenessRecorded?` | `boolean` | `false`                     | Emit a finite duration in live fMP4 initialization segments so Chromium treats the MSE stream as recorded instead of enabling its low-delay video renderer. |
